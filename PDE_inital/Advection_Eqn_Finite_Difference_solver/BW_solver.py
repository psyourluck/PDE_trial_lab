...
class beamWarming:
    ...
    def initializeParams(self):
        ...
        self.alpha3 = np.pi*self.dt/(2*self.dx)
    ...
    def solve_and_plot(self, plot=True):
        ...
        for i in range(self.nsteps-1):
            ...
            if self.prb != 4:
                ...
                for j in range(2, self.N):
                    self.unp1[j] = self.u[j] - self.alpha3 * (3 * self.u[j]**2 - 4 * self.u[j - 1]**2 + self.u[j - 2]**2) + \
                                   2*self.alpha3**2 * ((self.u[j]+self.u[j-1])*(self.u[j]**2 - self.u[j-1]**2) - \
                                    (self.u[j-1]+self.u[j-2])*(self.u[j-1]**2 - self.u[j-2]**2))
            ...
            if self.prb == 0:
                ...
                self.unp1[1] = self.unp1[1] - self.alpha1 * (3 * self.unp1[1] - 3) + self.alpha2 * (self.unp1[1] - 1)
                uexact = (self.x - self.v * tc <= 0)+0.
            elif self.prb == 2:
                ## periodic BC
                self.unp1[0] = self.unp1[self.N-1]
                self.unp1[1] = self.u[1] - self.alpha1 * (3 * self.u[1] - 4 * self.u[0] + self.u[self.N-2]) + \
                               self.alpha2 * (self.u[1] - 2 * self.u[0] + self.u[self.N-2])
                ...
                self.unp1[0] = self.unp1[self.N-1]
                self.unp1[1] = self.u[1] - self.alpha3 * (3 * self.u[1] ** 2 - 4 * self.u[0] ** 2 + self.u[self.N-2] ** 2) + \
                               2 * self.alpha3 ** 2 * ((self.u[1] + self.u[0]) * (self.u[1] ** 2 - self.u[0] ** 2) - \
                                           (self.u[0] + self.u[self.N-2]) * (self.u[0] ** 2 - self.u[self.N-2] ** 2))
...
def main():
    #sim = beamWarming(101, 0.5, 0.005,1,yinf=-0.3,ysup=1.4,dynamic=False)
    #sim = beamWarming(401, 0.5, 0.00125, 1, yinf=-0.3, ysup=1.4, dynamic=False)
    #sim = beamWarming(201, 8, 0.004, 2, yinf=-1, ysup=1, dynamic=False)
    #sim = beamWarming(501, 8, 0.0032, 3, yinf=-1, ysup=1, dynamic=False, xmin=-1)
    #sim = beamWarming(201, 0.05, 0.01, 4, yinf=-1, ysup=1, dynamic=False, exact=False)
    # sim = beamWarming(1025, 0.015, 0.003, 4, yinf=-1, ysup=1, dynamic=False, exact=False)
    # sim.solve_and_plot()
    # plt.show()
    elltwo_error_box = []
    sup_error_box = []
    ...
    for i in range(2, 9):
        n = int(np.exp2(i))
        dt = 0.001
        sim = beamWarming(n + 1, 0.015, dt, 4, yinf=-1, ysup=1, dynamic=False, exact=False)
        ...
        if n == 128:
            dt == 0.0001
    np.save('BW_2', np.array(elltwo_error_box))
    np.save('BW_sup', np.array(sup_error_box))
