...
for rectan in Rectangle_gen(n)[0]:
    ...
    for i in range(0, 4):
        row = []
        for j in range(0, 4):
            row.append(dblquad(lambda x, y: gen.shape_rec(x, y, i) * np.dot(gen.grad_rec(x, y, j), inverse_jacobian)[1] \
            ...
            if lagrange_boundary[lagrange_points.index(temp_i)] == 2 and lagrange_boundary[
                ...
                if temp_i[1] == 1 and temp_j[1] == 1:
                    bound_matrix[lagrange_points.index(temp_i)][lagrange_points.index(temp_j)] += \
                        quad(lambda x: gen.shape_rec(x, 1, i) * gen.shape_rec(x, 1, j) * scalar_jacobian_bound, 0, 1)[0]
                    # stiffness_matrix[lagrange_points.index(temp_i)][lagrange_points.index(temp_j)] += \
                    #    4*quad(lambda x: gen.shape_rec(x, 1, i) * gen.shape_rec(x, 1, j) * scalar_jacobian_bound, 0, 1)[0]
                    # dblquad(lambda x, y: gen.shape_rec(x, y, i) * gen.shape_rec(x, y, j) * scalar_jacobian, 0, 1,
                    #         lambda x: 0, lambda x: 1)[0]
                if temp_i[1] == 0 and temp_j[1] == 0:
                    bound_matrix[lagrange_points.index(temp_i)][lagrange_points.index(temp_j)] += \
                        quad(lambda x: gen.shape_rec(x, 0, i) * gen.shape_rec(x, 0, j) * scalar_jacobian_bound, 0, 1)[0]
                    # stiffness_matrix[lagrange_points.index(temp_i)][lagrange_points.index(temp_j)] += \
                    #    4*quad(lambda x: gen.shape_rec(x, 0, i) * gen.shape_rec(x, 0, j) * scalar_jacobian_bound, 0, 1)[0]
                if temp_i[0] == 1 and temp_j[0] == 1:
                    bound_matrix[lagrange_points.index(temp_i)][lagrange_points.index(temp_j)] += \
                        quad(lambda y: gen.shape_rec(1, y, i) * gen.shape_rec(1, y, j) * scalar_jacobian_bound, 0, 1)[0]
                    #stiffness_matrix[lagrange_points.index(temp_i)][lagrange_points.index(temp_j)] += \
                    #    4*quad(lambda y: gen.shape_rec(1, y, i) * gen.shape_rec(1, y, j) * scalar_jacobian_bound, 0, 1)[0]
            lagrange_points = np.array(lagrange_points)
    # print mass_matrix
...
#                            np.dot(actual_stiffness_matrix, forcing_grad) + np.dot(actual_bound_matrix, forcing_bound))
