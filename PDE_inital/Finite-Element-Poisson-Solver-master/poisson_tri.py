## Solve the 2D poisson equation with mixed boundary condition using FEM
## with type (1) triangles as finite elements
...
for triangle in Triangle_gen(n)[0]:
    ...
    for test_points in gen.lagrange_test_tri:
        lagrange_points.append(np.dot(jacobian, np.transpose(test_points))+np.array(triangle[0]))
...
for triangle in Triangle_gen(n)[0]:
    ...
    for test_points in gen.lagrange_test_tri:
        transformed_test_points.append(np.dot(jacobian, np.transpose(test_points))+np.array(triangle[0]))
    ...
    for i in range(0, 3):
        row = []
        for j in range(0, 3):
            row.append(dblquad(lambda x, y: gen.shape_tri(x, y, i) * np.dot(gen.grad_tri(x, y, j), inverse_jacobian)[1] \
            ...
            if lagrange_boundary[lagrange_points.index(temp_i)] == 2 and lagrange_boundary[
                ...
                if temp_i[1] == 1 and temp_j[1] == 1 and count%2 == 1:
                    bound_matrix[lagrange_points.index(temp_i)][lagrange_points.index(temp_j)] += \
                        quad(lambda x: gen.shape_tri(x, 1-x, i) * gen.shape_tri(x, 1-x, j) * scalar_jacobian_bound, 0, 1)[0]
                    stiffness_matrix[lagrange_points.index(temp_i)][lagrange_points.index(temp_j)] += \
                ...
                if temp_i[1] == 0 and temp_j[1] == 0 and count%2==0:
                    bound_matrix[lagrange_points.index(temp_i)][lagrange_points.index(temp_j)] += \
                        quad(lambda x: gen.shape_tri(x, 0, i) * gen.shape_tri(x, 0, j) * scalar_jacobian_bound, 0, 1)[0]
                    stiffness_matrix[lagrange_points.index(temp_i)][lagrange_points.index(temp_j)] += \
                ...
                if temp_i[0] == 1 and temp_j[0] == 1 and count%2==1:
                    bound_matrix[lagrange_points.index(temp_i)][lagrange_points.index(temp_j)] += \
                        quad(lambda y: gen.shape_tri(y, 0, i) * gen.shape_tri(y, 0, j) * scalar_jacobian_bound, 0, 1)[0]
                    stiffness_matrix[lagrange_points.index(temp_i)][lagrange_points.index(temp_j)] += \
...
solution = np.linalg.solve(actual_stiffness_matrix, np.dot(actual_mass_matrix, forcing) + \
                           np.dot(actual_mass_grad_matrix, forcing_grad) + np.dot(actual_bound_matrix, forcing_bound))
...
print('ell2 error: {:.6f}, sup error: {:.6f}'.format(elltwo_error, sup_error))
