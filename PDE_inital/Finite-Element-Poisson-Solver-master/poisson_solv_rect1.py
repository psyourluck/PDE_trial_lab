...
for n in range(26, 27):
    ...
    for i in range(0, len(lagrange_points)):
        elltwo_error += ((analytic_soln(lagrange_points[i][0], lagrange_points[i][1]) - full_solution[i]) ** 2 / float(n ** 2))
...
#
# np.save('rect2_ell2-', np.array(elltwo_error_box))
# np.save('rect2_sup-', np.array(sup_error_box))
# np.save('rect2_h1-', np.array(H1_error_box))


## the following code draws the 3D plot for the solution
