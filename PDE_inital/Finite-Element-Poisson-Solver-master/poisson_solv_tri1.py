...
for n in range(2,21):
    ...
    for triangle in Triangle_gen(n)[0]:
        ...
        for i in range(0, 3):
            ...
            H1_matrix_x[count, lagrange_points.index(temp_i)] = np.dot(gen.grad_tri(1 / 3, 1 / 3, i), inverse_jacobian)[
                0]
            H1_matrix_y[count, lagrange_points.index(temp_i)] = np.dot(gen.grad_tri(1 / 3, 1 / 3, i), inverse_jacobian)[
                1]
            lagrange_points = np.array(lagrange_points)
                    ...
                    if temp_i[1] == 1 and temp_j[1] == 1 and count % 2 == 1:
                        bound_matrix[lagrange_points.index(temp_i)][lagrange_points.index(temp_j)] += \
                            quad(lambda x: gen.shape_tri(x, 1 - x, i) * gen.shape_tri(x, 1 - x,
                                ...
                                0]
                        stiffness_matrix[lagrange_points.index(temp_i)][lagrange_points.index(temp_j)] += \
                            4 * quad(lambda x: gen.shape_tri(x, 1 - x, i) * gen.shape_tri(x, 1 - x,
                                                                                          j) * scalar_jacobian_bound, 0,
                                     1)[0]
...
np.save('tri1_ell2', np.array(elltwo_error_box))
np.save('tri1_sup', np.array(sup_error_box))
np.save('tri1_h1', np.array(H1_error_box))
